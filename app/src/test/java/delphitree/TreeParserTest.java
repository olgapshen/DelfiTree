...
class TreeParserTest {
    ...
    void testMakeTree()
    {
        ...
        assertEquals(6, nodes.getNodeByName("uCEFTypes").getIfaceUses().size());
        assertEquals(0, nodes.getNodeByName("uCEFTypes").getImplUses().size());
        ...
        assertEquals(2, nodes.getNodeByName("uCEFSchemeRegistrar").getIfaceUses().size());
        assertEquals(1, nodes.getNodeByName("uCEFSchemeRegistrar").getImplUses().size());
        assertEquals(6, nodes.getNodeByName("uCEFApp").getIfaceUses().size());
        assertEquals(0, nodes.getNodeByName("uCEFApp").getImplUses().size());
        assertEquals(1, nodes.getNodeByName("uCEFConstants").getIfaceUses().size());
        assertEquals(0, nodes.getNodeByName("uCEFConstants").getImplUses().size());
